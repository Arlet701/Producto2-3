...
namespace COVID_DAL
{
    public class Eliminar
    {
        ...
        public void EliminarIncapac(string Periodo, int F_positivoProfe)
        {
            using (SqlConnection Con = new SqlConnection(CadCon))
            {
                Con.Open();
                using (SqlCommand Comando = new SqlCommand())
                {
                    ...
                    Comando.Parameters.AddWithValue("@pos", F_positivoProfe);

                    Comando.CommandText = "DELETE FROM Incapacidad WHERE Periodo = @Perio AND F_positivoProfe = @pos";
                    Comando.ExecuteNonQuery();
                }
                Con.Close();
                Con.Dispose();
